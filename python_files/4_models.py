...
def plot_confusion_matrices(cms_list, dataset_name):
    ...
    for idx, (model_name, cm) in enumerate(cms_list):
        ...
        axes[idx].set_title(f'{model_name}', fontsize=18, fontweight='bold', pad=12)
...
ax.set_title('Model Accuracy Comparison Across Datasets', fontsize=20, fontweight='bold', pad=15)
ax.set_xticks(x + width/2)
ax.set_xticklabels(subset['model'], rotation=45, ha='right', fontsize=13)
...
for i, dataset in enumerate(datasets):
    subset = results_df[results_df['dataset'] == dataset].sort_values('model')
    bars = ax.bar(x + i*width, subset['f1_score'], width,
           label=dataset.upper(), alpha=0.8)
    for bar in bars:
        height = bar.get_height()
        ax.annotate(f'{height:.3f}',
...
ax.set_title('Model F1-Score Comparison Across Datasets', fontsize=20, fontweight='bold', pad=15)
ax.set_xticks(x + width/2)
ax.set_xticklabels(subset['model'], rotation=45, ha='right', fontsize=13)
ax.tick_params(axis='y', labelsize=13)
ax.legend(fontsize=14, loc='upper right')
...
#3. Training Time vs Accuracy Trade-off (FIXED OVERLAPPING LABELS)
fig, ax = plt.subplots(figsize=(18, 10))

colors = {'ds2os': '#3182CE', 'nsl_kdd': '#38A169'}
...
for dataset in datasets:
    ...
    ax.scatter(subset['train_time_sec'], subset['accuracy'],
               s=220, alpha=0.85, label=dataset.upper(),
               color=colors[dataset], marker=markers[dataset],
               edgecolors='white', linewidth=1.5, zorder=5)

#Set axis limits with generous margins for label placement
ax.set_xlim(-8, 78)
ax.set_ylim(0.73, 1.04)
...
#sort into two groups: DS2OS (top) and NSL_KDD (bottom)
ds2os_points = sorted([p for p in all_points if p['dataset'] == 'ds2os'], key=lambda p: p['x'])
nslkdd_points = sorted([p for p in all_points if p['dataset'] == 'nsl_kdd'], key=lambda p: p['x'])

ds2os_label_xs = np.linspace(0, 72, len(ds2os_points))
for i, p in enumerate(ds2os_points):
    ax.annotate(
        p['model'],
        xy=(p['x'], p['y']),
        xytext=(ds2os_label_xs[i], 1.035),
        fontsize=11, fontweight='bold', color='#2B6CB0',
        ha='center', va='bottom',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='#EBF8FF',
                  edgecolor='#3182CE', alpha=0.9),
        arrowprops=dict(arrowstyle='->', color='#3182CE', alpha=0.6, lw=1.0))

nsl_label_xs = np.linspace(0, 72, len(nslkdd_points))
for i, p in enumerate(nslkdd_points):
    ax.annotate(
        p['model'],
        xy=(p['x'], p['y']),
        xytext=(nsl_label_xs[i], 0.735),
        fontsize=11, fontweight='bold', color='#276749',
        ha='center', va='top',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='#F0FFF4',
                  edgecolor='#38A169', alpha=0.9),
        arrowprops=dict(arrowstyle='->', color='#38A169', alpha=0.6, lw=1.0))

ax.set_xlabel('Training Time (seconds)', fontsize=16, fontweight='bold')
ax.set_ylabel('Accuracy', fontsize=16, fontweight='bold')
ax.set_title('Computational Cost vs Performance Trade-off', fontsize=20, fontweight='bold', pad=15)
ax.legend(fontsize=14, markerscale=1.2, loc='center right')
...
sns.heatmap(heatmap_data, annot=True, fmt='.4f', cmap='RdYlGn',
            ...
            annot_kws={'size': 16, 'fontweight': 'bold'})
ax.set_title('Accuracy Heatmap: Models x Datasets', fontsize=20, fontweight='bold', pad=15)
