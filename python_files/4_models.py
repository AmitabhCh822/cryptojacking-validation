...
ax.set_title('Model Accuracy Comparison Across Datasets', fontsize=22, fontweight='bold', pad=15)
...
ax.set_title('Model F1-Score Comparison Across Datasets', fontsize=22, fontweight='bold', pad=15)
ax.set_xticks(x + width/2)
ax.set_xticklabels(subset['model'], rotation=45, ha='right', fontsize=16)
ax.tick_params(axis='y', labelsize=16)
ax.legend(fontsize=16, loc='center right')
...
ds2os_label_xs = np.linspace(2, 74, len(ds2os_points))
for i, p in enumerate(ds2os_points):
    ax.annotate(
        p['model'],
        xy=(p['x'], p['y']),
        xytext=(ds2os_label_xs[i], 1.055),
        fontsize=15, fontweight='bold', color='#2B6CB0',
        ha='center', va='bottom',
        bbox=dict(boxstyle='round,pad=0.4', facecolor='#EBF8FF',
                  edgecolor='#3182CE', alpha=0.9),
        arrowprops=dict(arrowstyle='->', color='#3182CE', alpha=0.6, lw=1.2))

nsl_label_xs = np.linspace(2, 74, len(nslkdd_points))
for i, p in enumerate(nslkdd_points):
    ax.annotate(
        p['model'],
        xy=(p['x'], p['y']),
        xytext=(nsl_label_xs[i], 0.71),
        fontsize=15, fontweight='bold', color='#276749',
        ha='center', va='top',
        bbox=dict(boxstyle='round,pad=0.4', facecolor='#F0FFF4',
                  edgecolor='#38A169', alpha=0.9),
        arrowprops=dict(arrowstyle='->', color='#38A169', alpha=0.6, lw=1.2))

ax.set_xlabel('Training Time (seconds)', fontsize=18, fontweight='bold')
ax.set_ylabel('Accuracy', fontsize=18, fontweight='bold')
ax.set_title('Computational Cost vs Performance Trade-off', fontsize=22, fontweight='bold', pad=15)
ax.legend(fontsize=16, markerscale=1.4, loc='center right')
...
ax.set_title('Accuracy Heatmap: Models x Datasets', fontsize=22, fontweight='bold', pad=15)
